...
GREEN = "FF1D9E75"; RED = "FFD94F3D"; YELLOW = "FFF5A623"
...
for i, h in enumerate(["Categoría","Activo","Archivos","Frames RFD","Inst. DS","Internos","Familias únicas DS","Adopción %"], 1):
    hdr(ws1, 4, i, h)
row = 5
for team in report["teams"]:
    ...
    row += 1
for col, w in zip(range(1,9), [18,24,10,12,12,12,16,12]):
...
for i, h in enumerate(["Equipo","Categoría","Quarter","Fecha","Adopción %","Frames RFD","Inst. DS","Total"], 1):
    hdr(ws5, 1, i, h)
row = 2
for entry in sorted(report.get("history",[]), key=lambda x: (x["team"], x["quarter"])):
    ...
    dat(ws5, row, 6, entry.get("totalRfdFrames",0), align="center")
    dat(ws5, row, 7, entry.get("totalDs",0), align="center", color="FF1A7FA8")
    dat(ws5, row, 8, entry.get("totalInstances",0), align="center")
    row += 1
for col, w in zip(range(1,9), [24,18,12,14,12,12,12,12]):
...
async function run() {
  ...
  for (const team of config.teams) {
    ...
    // Aggregate unique families across files
    const familyMap = {};
  ...
  console.log("\n=== Summary ===");
  for (const team of teamsData) {
    console.log(`[${team.category}] ${team.name}: ${team.adoptionRate}% | ${team.totalRfdFrames} RFD | ${team.uniqueFamiliesCount} familias únicas`);
  }
